//===- ORISCAsmPrinter.cpp ORISC LLVM Assembly Printer ------------------===//
//
...
// of machine-dependent LLVM code to GAS-format ORISC assembly language.
//
//===----------------------------------------------------------------------===//
...
void ORISCAsmPrinter::emitInstruction(const MachineInstr *MI) {
...
bool ORISCAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
...
bool ORISCAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
                                             unsigned OpNo,
  ...
  OS << ORISCInstPrinter::getRegisterName(Base.getReg());
  OS << ", ";
  OS << Offset.getImm();
...
MCOperand
ORISCAsmPrinter::lowerSymbolOperand(const MachineOperand &MO,
...
void ORISCAsmPrinter::lowerToMCInst(const MachineInstr *MI,
                                     MCInst &OutMI) const {
...
MCOperand ORISCAsmPrinter::lowerOperand(const MachineOperand &MO,
                                         unsigned Offset) const {
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeORISCAsmPrinter() {
  RegisterAsmPrinter<ORISCAsmPrinter> A(getTheORISCTarget());
}